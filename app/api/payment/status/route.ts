...
export async function POST(request: Request) {
  try {
    ...
    if (isBoostPayment) {
      ...
      if (updateTxError) {
        console.error('Gagal memperbarui status transaksi boost:', updateTxError);
        throw new Error('Gagal memperbarui status transaksi boost di database');
      }
      ...
      try {
        ...
        console.error('Gagal mengaktifkan boost event:', boostError);
      }
