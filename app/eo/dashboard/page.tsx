...
export default function EODashboard() {
  ...
  const handleSubmitEvent = async (e: React.FormEvent) => {
    ...
    for (let cat of categories) {
      if (!cat.name || !cat.price || !cat.stock) return alert("Bro, data kategori tiket lo masih ada yang kosong!");
    }
    ...
    try {
      ...
      if (editingEventId) {
        ...
        if (err) throw err;
        alert("Peringatan: Edit event saat ini tidak mengubah daftar kategori tiket!");
      } else {
...
>
  <QrCode size={20} /> Scan Check-In
      ...
      {/* CONTENT */}
      <div className="flex-1 p-6 md:p-12 bg-[#FCFAF1] overflow-y-auto max-h-screen">
        <div className="max-w-7xl mx-auto space-y-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 text-left">
            <div>
              ...
              <p className="text-slate-500 font-bold italic text-lg mt-2">Yo {eoName.split(' ')[0]}, kelola panggung & pantau cuan lo di sini.</p>
            </div>
          ...
          {/* ⚡ STATISTIK CUAN & TIKET */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0_0_#000] flex items-center gap-6">
              <div className="bg-emerald-400 p-4 border-4 border-black shadow-[4px_4px_0_0_#000]">
                <Banknote size={40} className="text-black" strokeWidth={2.5} />
              </div>
              <div>
                <p className="text-xs font-black uppercase text-slate-400 tracking-widest italic mb-1">Total Pendapatan</p>
                <p className="text-4xl font-black italic tracking-tighter text-emerald-600">{formatRupiah(totalRevenue)}</p>
              </div>
            </div>
            <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0_0_#000] flex items-center gap-6">
              <div className="bg-amber-400 p-4 border-4 border-black shadow-[4px_4px_0_0_#000]">
                <Users size={40} className="text-black" strokeWidth={2.5} />
              </div>
              <div>
                <p className="text-xs font-black uppercase text-slate-400 tracking-widest italic mb-1">Tiket Terjual</p>
                <p className="text-4xl font-black italic tracking-tighter">{totalTicketsSold} <span className="text-xl">Lembar</span></p>
              </div>
            </div>
          </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-black text-white text-[10px] font-black uppercase tracking-[0.2em]">
                    ...
                    <th className="p-6 text-center">Aksi & Tools</th>
                  </tr>
                </thead>
                  ...
                  {isLoading ? (
                    ...
                    <tr key={event.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <div className="flex gap-2">
                              ...
                              <span className="text-[8px] font-black px-3 py-1 border-2 border-black uppercase italic bg-slate-200">
                      ...
                      <td className="p-6 border-r-4 border-black font-black uppercase italic text-xs space-y-2">
                        <div className="flex items-center gap-2"><Calendar size={14} className="text-red-500" /> {event.date} {event.end_date ? `- ${event.end_date}` : ''}</div>
                      ...
                      <td className="p-6">
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex items-center justify-center gap-3 text-black">
                            <button onClick={() => openEditModal(event)} className="flex-1 bg-white border-2 border-black p-2 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all flex justify-center">
                              <Edit2 size={16} strokeWidth={3} />
                            </button>
                            <button onClick={() => { if(confirm("Hapus?")) supabase.from("events").delete().eq("id", event.id).then(() => fetchMyEvents(eoId!)) }} className="flex-1 bg-white border-2 border-black p-2 text-red-500 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all flex justify-center">
      ...
      {/* MODAL (BUAT/EDIT EVENT) - Tetap Sama Persis */}
      {isModalOpen && (
            ...
            <form onSubmit={handleSubmitEvent} className="text-left space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50 border-4 border-black p-6">
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="text-xs font-black uppercase text-slate-400 mb-2 block italic">Kategori Event</label>
                    <select 
                      className="w-full p-4 border-4 border-black bg-white font-black italic uppercase outline-none focus:bg-amber-100 text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] cursor-pointer"
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div onClick={() => fileInputRef.current?.click()} className="flex-1 border-4 border-dashed border-black bg-white flex flex-col items-center justify-center cursor-pointer hover:bg-slate-200 transition-colors text-center p-2">
                        <p className="text-[10px] font-black uppercase tracking-widest text-black">{imageFile ? imageFile.name : "Upload / Ganti Poster"}</p>
                ...
                <div className="space-y-6 text-black flex flex-col justify-between">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" required className="w-full p-4 border-4 border-black bg-white font-black text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] cursor-text" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} />
                    </div>
                    
                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-400 mb-1 block italic">Tgl Selesai (Opsional)</label>
                      <input type="date" className="w-full p-4 border-4 border-black bg-white font-black text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] cursor-text" value={formData.end_date} onChange={(e) => setFormData({...formData, end_date: e.target.value})} />
                    </div>

                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-400 mb-1 block italic">Jam Mulai</label>
                      <input type="time" className="w-full p-4 border-4 border-black bg-white font-black text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] cursor-text" value={formData.start_time} onChange={(e) => setFormData({...formData, start_time: e.target.value})} />
                    </div>
                    <div>
                      <label className="text-[10px] font-black uppercase text-[#FF6B6B] mb-1 block italic">Maks Beli/Akun</label>
                  ...
                  <div>
                    ...
                    <textarea rows={4} className="w-full p-4 border-4 border-black bg-white font-medium text-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] custom-scrollbar" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-amber-400 border-8 border-black p-6 shadow-[8px_8px_0_0_rgba(0,0,0,1)] space-y-4">
                ...
                {editingEventId && <p className="text-xs font-bold bg-white text-red-600 p-2 border-2 border-black mb-4 inline-block shadow-[4px_4px_0_0_#000]">Edit kategori sementara tidak tersedia. Hapus event jika ingin ubah harga.</p>}
                  ...
                  {categories.map((cat, index) => (
                    <div key={cat.id} className="bg-white border-4 border-black p-4 relative flex flex-col md:flex-row gap-4 items-center group shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                      ...
                      <div className="flex-1 w-full mt-2 md:mt-0">
                        <label className="text-[9px] font-black uppercase text-slate-400 italic">Nama Kategori</label>
