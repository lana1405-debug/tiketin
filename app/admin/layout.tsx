...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!mounted || isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-zinc-950 text-black dark:text-white font-black italic">
        <div className="p-8 border-8 border-black dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-[12px_12px_0px_0px_rgba(109,74,255,1)] dark:shadow-[12px_12px_0px_0px_var(--primary-color)] flex flex-col items-center">
  ...
  const SidebarContent = () => (
    <>
      ...
      {/* MENU LIST */}
      <nav className="flex-1 p-4 lg:p-6 space-y-3 lg:space-y-4 overflow-y-auto custom-scrollbar text-left bg-slate-50 dark:bg-zinc-950">
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`group flex items-center gap-4 px-4 lg:px-6 py-4 lg:py-5 border-4 border-black dark:border-zinc-700 font-black uppercase italic text-[10px] tracking-widest transition-all ${
                isActive 
                ? "bg-black dark:bg-zinc-900 text-white dark:text-zinc-50 shadow-[6px_6px_0px_0px_rgba(109,74,255,1)] dark:shadow-[6px_6px_0px_0px_var(--primary-color)] translate-x-[-4px] translate-y-[-4px]" 
                : "bg-white dark:bg-zinc-900 text-black dark:text-zinc-50 hover:bg-slate-100 dark:hover:bg-zinc-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.15)] hover:shadow-none hover:translate-x-1 hover:translate-y-1"
              }`}
            >
              <item.icon size={20} strokeWidth={3} className={isActive ? "text-amber-400" : "text-black dark:text-zinc-50"} />
              {item.name}
            </Link>
          );
        })}
    ...
    <div className={`flex min-h-screen bg-white dark:bg-zinc-950 ${poppins.className} text-black dark:text-zinc-50 admin-layout`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden sticky top-0 z-50 bg-[#6D4AFF] border-b-4 border-black dark:border-zinc-750 h-16 flex items-center justify-between px-4 shrink-0">
          ...
          >
            <Menu size={22} strokeWidth={3} className="text-[#6D4AFF] dark:text-zinc-50" />
          </button>
          <div className="flex items-center gap-2">
            <div className="bg-white dark:bg-zinc-800 p-1.5 border-2 border-black dark:border-zinc-700">
              <TicketIcon className="text-[#6D4AFF] dark:text-zinc-50" size={18} strokeWidth={4} />
            </div>
        ...
        {/* ── PAGE CONTENT ── */}
        <main className="flex-1 p-4 sm:p-8 lg:p-16 bg-white dark:bg-zinc-950 overflow-y-auto relative">
